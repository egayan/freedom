...
<body>
    ...
    <button id="toggleButton" class="button">切り替え</button>
    <div class="title">
    ...
    <table id="rankingList">
        ...
        <th class="shohin">商品</th>
        </tr>
        <!-- 週間ランキングの内容をここに追加 -->
        <?php foreach($weeklyRanking as $key => $rank):?>
        <tr>
            <td class="rank"><?php echo $key+1;?>位</td>
            <td><a href="detail.php?id=<?php echo $rank['shohin_id'];?>">
            <image src="image/<?php echo $rank['image'];?>" alt="商品画像" class="g"></a></td>
        </tr>
        <?php endforeach;?>
    </table>
    <!-- 総合ランキングの内容も同様に追加 -->
